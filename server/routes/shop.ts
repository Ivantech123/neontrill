...
router.get("/", async (req, res) => {
  try {
    ...
    const convertedGifts = telegramGifts.map((gift) =>
      telegramGiftsService.convertToGameItem(gift),
    );
  ...
  try {
    ...
    if (telegramGift) {
      ...
      if (!isAvailable) {
        return res
          .status(400)
          .json({ error: "Gift is sold out or unavailable" });
      }
  ...
  try {
    ...
    if (telegramGift) {
      ...
      return res.status(400).json({
        error:
          "Telegram gifts cannot be sold. They can be converted to Stars instead.",
        canConvert: true,
